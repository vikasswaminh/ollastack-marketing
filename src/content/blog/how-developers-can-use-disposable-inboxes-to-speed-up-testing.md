...
<div class="faq-accordion-wrap">
  <details class="faq-item">
    <summary class="faq-question">What is a disposable inbox in software testing?</summary>
    <div class="faq-answer">
      A disposable inbox is a temporary, real email address created specifically for a test run. It can receive actual email over SMTP and be read over HTTP, and it is discarded or expires once the test finishes, so it never needs to be shared across multiple tests or reused between runs.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">How is a disposable inbox different from Mailhog or Mailpit?</summary>
    <div class="faq-answer">
      Mailhog and Mailpit intercept outgoing SMTP connections locally, which proves your application attempted to send a message. They never deliver anything anywhere. A disposable inbox receives real mail, which means it can confirm delivery, authentication, and rendering in a way a local SMTP catcher cannot.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">Why does polling with sleep cause flaky tests?</summary>
    <div class="faq-answer">
      A fixed sleep assumes a constant delivery time. If the real delivery is slower than the sleep duration, the test fails even though the email would have arrived a moment later. If the sleep is set too long to compensate, every test run wastes time waiting for nothing. Long polling avoids both problems by returning the instant a message arrives, up to a generous timeout.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">Can I use a disposable inbox in Docker without adding an SMTP service?</summary>
    <div class="faq-answer">
      Yes. Since the inbox is created and read entirely over HTTP, the only requirement inside a container is outbound internet access and an API token passed as an environment variable. There is no port to expose and no additional service to define.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">Does this work with GitHub Actions and GitLab CI?</summary>
    <div class="faq-answer">
      Yes. Both platforms support secret environment variables, and since the entire pattern is plain HTTP requests, it runs identically on any CI provider with no service containers or special runner configuration required.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">How do I avoid parallel tests by reading each other's emails?</summary>
    <div class="faq-answer">
      Create a new, unique inbox for every test rather than sharing one address across your suite. This guarantees isolation with no possibility of a race condition, since no two tests are ever assigned to the same address at the same time.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">Can I test OTP codes with a disposable inbox?</summary>
    <div class="faq-answer">
      Yes, and it is one of the most common uses. Create an inbox, trigger the OTP send, long poll for the message, and read the extracted numeric code directly rather than parsing it out of the raw email body yourself.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">Do disposable inboxes work with Playwright and Cypress?</summary>
    <div class="faq-answer">
      Yes. Both frameworks can call the same HTTP endpoints used anywhere else, either directly inside a test or wrapped in a fixture or custom command, to create an inbox, wait for a message, and extract a code or link before continuing the browser flow.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">Are disposable inboxes safe to use with real customer data?</summary>
    <div class="faq-answer">
      They should generally be used with synthetic test data rather than real customer information. Treat the API token used to access them as a sensitive credential, store it as a CI secret, and use scoped tokens and short retention windows where your provider supports them.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">Do AI agents need disposable inboxes too?</summary>
    <div class="faq-answer">
      Yes. An agent that signs up for a service or completes an email verification step on a user's behalf needs an address it can receive mail at and a reliable way to read the message, which is the exact same pattern used in automated testing, just called from agent code instead of a test runner.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">What should I look for in a disposable inbox provider for CI?</summary>
    <div class="faq-answer">
      A long poll or webhook based wait mechanism, automatic extraction of codes and links, a test mode that skips spam filtering, cheap and fast inbox creation for isolation, and a plain HTTP API that works the same from any language or CI provider.
    </div>
  </details>

  <details class="faq-item">
    <summary class="faq-question">Is it worth switching from a shared test Gmail account?</summary>
    <div class="faq-answer">
      For any suite running more than a handful of email dependent tests, or running tests in parallel, yes. A shared inbox becomes a source of race conditions and flaky failures as soon as more than one test tries to read from it around the same time, and that problem only gets worse as a suite grows.
    </div>
  </details>
</div>
